        ...
        <x-form wire:submit.prevent="submit" class="space-y-4">
            ...
            @if ($step === 1)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-input label="Data de nascimento" type="date" wire:model="user_data_birth_date" name="user_data_birth_date"/>
                </div>
            @endif
            ...
            @if ($step === 3)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        <x-input label="Email de Contato" wire:model.live.debounce.1000ms="company_data_contact_email" required name="company_data_contact_email"/>
                    </div>

                    <x-input label="Número de Celular" x-mask="(99) 99999-9999" wire:model="company_data_phone_number" inputmode="numeric" name="company_data_phone_number"/>
                    <x-input label="Número de Telefone Fixo" x-mask="(99) 9999-9999" wire:model="company_data_landline_number" inputmode="numeric" name="company_data_landline_number"/>

                    <x-input label="Logomarca" type="file" wire:model="company_data_logo_picture_path" name="company_data_logo_picture_path" accept="image/jpeg,image/jpg,image/png"/>
                    <x-input label="Data de Fundação" type="date" wire:model="company_data_foundation_date" name="company_data_foundation_date"/>

                    <x-checkbox label="O endereço da empresa é o mesmo no qual resido." wire:model.live="company_data_company_same_user_address" name="company_data_company_same_user_address"/>
                </div>
            ...
            @if ($step === 4 && !$company_data_company_same_user_address)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-input label="CEP" x-mask="99999-999" wire:model.live.debounce.1000ms="company_address_data_cep" inputmode="numeric" required name="company_address_data_cep"/>
                    <x-input label="Número" wire:model.live.debounce.1000ms="company_address_data_building_number" inputmode="numeric" maxlength="5" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 5)" required name="company_address_data_building_number"/>

                    <div class="md:col-span-2">
                        <x-input label="Rua" wire:model.live.debounce.1000ms="company_address_data_street" oninput="this.value = this.value.replace(/[^a-zA-ZÀ-ÿ0-9\s,.'-]/g, '')" required name="company_address_data_street"/>
                    </div>

                    <div class="md:col-span-2">
                        <x-input label="Bairro" wire:model.live.debounce.1000ms="company_address_data_neighborhood" oninput="this.value = this.value.replace(/[^a-zA-ZÀ-ÿ0-9\s'-]/g, '')" required name="company_address_data_neighborhood"/>
                    </div>

                    <x-input label="Cidade" wire:model.live.debounce.1000ms="company_address_data_city" oninput="this.value = this.value.replace(/[^a-zA-ZÀ-ÿ\s'-]/g, '')" required name="company_address_data_city"/>
                    <x-input label="Estado" wire:model.live.debounce.1000ms="company_address_data_state" oninput="this.value = this.value.replace(/[^a-zA-ZÀ-ÿ\s'-]/g, '')" required name="company_address_data_state"/>

                    <div class="md:col-span-2">
                        <x-input label="Complemento" wire:model="company_address_data_complement" class="h-14" name="company_address_data_complement"/>
                    </div>
                </div>
            @endif
