...
<head>
    ...
    {{-- A variável $title é passada como um atributo no <x-layouts.app> --}}
    ...
    <style>
        /* [Seu código CSS de Sidebar e Navbar aqui] */
        :root { --beige-color: #F8F2EC; }
        body { background-color: var(--beige-color); font-family: 'Inter', sans-serif; }
        /* 1. SIDEBAR: Configuração de Hover e Fixação */
        .sidebar { width: 4rem; transition: width 0.3s ease-in-out; background-color: var(--beige-color); }
        .sidebar:hover, .sidebar.sidebar-fixed { width: 14rem; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05); }
        /* Ocultar texto e sub-itens por padrão */
        .sidebar-text, .submenu-item { display: none; opacity: 0; transition: opacity 0.15s ease-in-out; }
        /* Mostrar texto e sub-itens no hover OU quando fixada */
        .sidebar:hover .sidebar-text, .sidebar:hover .submenu-item, .sidebar.sidebar-fixed .sidebar-text, .sidebar.sidebar-fixed .submenu-item { display: inline-block; opacity: 1; }
        .submenu { max-height: 0; overflow: hidden; transition: max-height 0.3s ease-in-out; padding-top: 0; padding-bottom: 0; }
        .submenu.expanded { max-height: 200px; padding-top: 0.5rem; padding-bottom: 0.5rem; }
        /* 4. Dropdown do Perfil */
        .profile-dropdown { display: none; }
        .profile-menu.profile-menu-open .profile-dropdown { display: block; }
        /* Adicionar transição para rotação da seta */
        .nav-item [data-toggle="submenu"] svg:last-child { transition: transform 0.2s; }
        .rotate-180 { transform: rotate(180deg); }
        .sidebar-icon { color: rgba(0, 0, 0, 0.5); transition: color 0.2s; }
        .sidebar:hover .nav-item:hover .sidebar-icon { color: rgba(0, 0, 0, 0.7); }
        /* Classes para os ícones da Navbar (para herdar a cor) */
        .navbar-icon path { stroke: currentColor; }
    </style>
</head>
<body class="flex h-screen overflow-hidden">

<aside id="sidebar" class="sidebar shadow-xl shadow-gray-100/50 flex flex-col p-4 border-r border-gray-200 z-20">
...
<nav class="flex-1 space-y-2">
    {{-- Estrutura de Menu do seu app.blade.php aqui --}}
    @php
        use Illuminate\Support\Str; // Importado aqui para garantir que funcione se não estiver no topo
        $menu = [
            'Relatórios' => [
            ...
            <div class="submenu @if($title === 'Relatórios') expanded @endif ml-2" id="submenu-{{ Str::slug($title) }}">
                <ul class="pt-1 space-y-1 text-base text-gray-700">
                    @foreach($item['sub'] as $subItem)
                        <a href="{{ $subItem === 'Adicionar novo' ? route('stock.create') : '#' }}">
...
<div class="flex flex-col flex-1 overflow-x-hidden overflow-y-auto">
    ...
    <header class="h-16 flex items-center justify-between border-b border-gray-100/50 shadow-sm px-6 sticky top-0 z-10" style="background-color: #F8F2EC;">

        <div class="flex items-center space-x-4">
            {{-- 1. ÍCONE DE TOGGLE DA SIDEBAR (SVG FORNECIDO) --}}
            <div id="navbarMenuToggle" class="p-2 text-gray-500 hover:text-gray-700 cursor-pointer rounded-lg hover:bg-stone-200">
                <svg class="w-6 h-6 navbar-icon" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M6.66675 11.6667H11.6667M33.3334 11.6667H18.3334M33.3334 28.3334H28.3334M6.66675 28.3334H21.6667M6.66675 20.0001H33.3334" stroke="currentColor" stroke-opacity="0.5" stroke-width="2" stroke-linecap="round"/>
                </svg>
            </div>
